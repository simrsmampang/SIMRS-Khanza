...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            satu_sehat_mapping_radiologi=false,satu_sehat_kirim_servicerequest_radiologi=false,hasil_endoskopi_hidung=false,satu_sehat_kirim_specimen_radiologi=false,bpjs_kompilasi_berkas_klaim=false,master_masalah_keperawatan_neonatus=false,
            ...
            catatan_observasi_restrain_nonfarma=false,catatan_observasi_ventilator=false,catatan_anestesi_sedasi=false,paket_mcu=false,master_paket_mcu=false,skrining_puma=false,satu_sehat_kirim_careplan=false,satu_sehat_kirim_medicationstatement=false,
    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.satu_sehat_kirim_specimen_radiologi,user.bpjs_kompilasi_berkas_klaim,user.master_masalah_keperawatan_neonatus,user.master_rencana_keperawatan_neonatus,user.penilaian_awal_keperawatan_ranap_neonatus,"+
                ...
                "user.catatan_observasi_ventilator,user.catatan_anestesi_sedasi,paket_mcu,master_paket_mcu,user.skrining_puma,user.satu_sehat_kirim_careplan,user.satu_sehat_kirim_medicationstatement,"+
                ...
                "user.sirkulasi_dapur2,user.verifikasi_penerimaan_dapur,user.nilai_penerimaan_vendor_dapur_perbulan,user.ringkasan_hutang_vendor_dapur,user.penilaian_psikologi_klinis from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    verifikasi_penerimaan_dapur=rs.getBoolean("verifikasi_penerimaan_dapur");nilai_penerimaan_vendor_dapur_perbulan=rs.getBoolean("nilai_penerimaan_vendor_dapur_perbulan");ringkasan_hutang_vendor_dapur=rs.getBoolean("ringkasan_hutang_vendor_dapur");penilaian_psikologi_klinis=rs.getBoolean("penilaian_psikologi_klinis");
                    setTampil();
                }       
    ...
    private void setTampil(){
        ...
        if("[L]Kompilasi Berkas Klaim BPJS".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[L]Kompilasi Berkas Klaim BPJS",bpjs_kompilasi_berkas_klaim});
        }
        ...
        if ("[M]Paket MCU".toLowerCase().contains(TCari.getText().toLowerCase())) {
            tabMode.addRow(new Object[] {false, "[M]Paket MCU", paket_mcu});
        }
        
        if ("[M]Master Paket MCU".toLowerCase().contains(TCari.getText().toLowerCase())) {
            tabMode.addRow(new Object[] {false, "[M]Master Paket MCU", master_paket_mcu});
        }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if ("[M]Paket MCU".equals(tbUser.getValueAt(i, 1).toString())) {
                Sequel.mengupdateSmc("user", "paket_mcu = ?", "id_user = aes_encrypt(?, 'nur')", tbUser.getValueAt(i, 2).toString(), TKd.getText());
            }

            if ("[M]Master Paket MCU".equals(tbUser.getValueAt(i, 1).toString())) {
                Sequel.mengupdateSmc("user", "master_paket_mcu = ?", "id_user = aes_encrypt(?, 'nur')", tbUser.getValueAt(i, 2).toString(), TKd.getText());
            }
