...
public final class SatuSehatReferensiNumerator extends javax.swing.JDialog {
    ...
    private void tampil() {
        ...
        try (PreparedStatement ps = koneksi.prepareStatement("select * from satu_sehat_referensi_numerator")) {
        ...
        try (FileReader fr = new FileReader("./cache/satusehatnumerator.iyem")) {
            ...
            if (root.isArray()) {
                if (TCari.getText().isBlank()) {
                    ...
                    for (JsonNode obj : root) {
                        if (obj.path("code").asText().contains(TCari.getText().trim())
                            || obj.path("name").asText().contains(TCari.getText().trim())) {
                            tabMode.addRow(new String[] {
                                obj.path("code").asText(),
                                obj.path("name").asText(),
                                obj.path("system").asText()
